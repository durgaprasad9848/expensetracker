    ...
    if (forgot) {
      ...
      if (isLogin) {
        ...
        //  ctx.storeToken(data.idToken);                                            //////////////////
          ...
          dispatch(login({email:enteredEmail.replace("@gmail.com", ""), token:data.idToken }));     //
           //  navigate("/Welcome"); //
  ...
  return (
    <section className={classes.auth}>
      {forgot ? (
        ...
        <h5>Enter the email with which you have registered.</h5>
      )}
      <form>
        ...
        <div className={classes.actions}>
          ...
          {!isLoading && (
            <button onClick={formSubmitHandler}>
              {!forgot ? "Rest password" : isLogin ? "Login" : "Create Account"}
            </button>
          )}
